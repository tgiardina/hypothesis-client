...
describe('NotebookFilters', () => {
  ...
  beforeEach(() => {
    ...
    $imports.$mock({
      './hooks/use-user-filter-options': fakeUseUserFilterOptions,
  ...
  it('should render a user filter with options', () => {
    ...
    const props = wrapper.find('SelectFilter').props();
  ...
  it('should render the filter with a selected option if a user filter is applied', () => {
    ...
    assert.deepEqual(wrapper.find('SelectFilter').props().selectedOption, {
      display: 'One User',
      value: 'oneuser',
    });
  });
